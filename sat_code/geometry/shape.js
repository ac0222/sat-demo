// shape super class
var Shape = function(centre, rotation, c1, c2, shapeType, 
...
Shape.prototype.move = function(deltaT) {
...
Shape.prototype.update = function(deltaT) {
	if (this.isActive()) {
		this.move(deltaT);
	}
}
...
Shape.getEdgeVectors = function(vertices) {
	var evs = [];
...
Shape.getEdgeOrthogonals = function(vertices) {
	var evs = Shape.getEdgeVectors(vertices);
	var ovs = [];
...
// rendering
Shape.renderPolygon = function(vertices, ctx, fillStyle) {
	ctx.beginPath();
...
Shape.renderCircle = function(radius, centre, ctx, fillStyle) {
	ctx.beginPath();
...
// Collision detection
Shape.projectShapeFromPoint = function(shape, point, direction) {
	var scalarProjections = [];
	if (shape.shapeType == "circle") {
		var circCentre = new Vector2D(shape.centre.x, shape.centre.y, "cartesian");
		var centreToPoint = circCentre.subtract(point);
		...
		var polyVecs = [];
		var polyVertices = shape.getVertices();
...
Shape.ppCollisionDetection = function(poly1, poly2) {
...
Shape.pcCollisionDetection = function(poly, circ) {
...
Shape.ccCollisionDetection = function(circ1, circ2) {
